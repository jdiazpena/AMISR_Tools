%Several Altitude profiles of AMISr for 2012 January 24 In order to follow
%time domain of patches
...
xcut=100;  
ycut=([100 200 300 400 500]);
ticks=16;
altitudeaxis=[140 700];
...
yindex=[yindex(1) yindex(2)-40 yindex(3)-80 yindex(4)-120 yindex(5)-160];

DataCut1=squeeze(Neinterptime(:,yindex(1),xindex,:));
DataCut2=squeeze(Neinterptime(:,yindex(2),xindex,:));
DataCut3=squeeze(Neinterptime(:,yindex(3),xindex,:));
DataCut4=squeeze(Neinterptime(:,yindex(4),xindex,:));
DataCut5=squeeze(Neinterptime(:,yindex(5),xindex,:));
...
fig=figure;
s1=subplot(5,1,5);
set(s1, 'Position', get(s1,'Position')-[0.05 0.075 -0.03 -0.055]);
originalSize1 = get(s1, 'Position');

h=pcolor(time,zaxis,DataCut1');
hold on;
...
% Plot ratios
s2=subplot(5,1,4);
set(s2, 'Position', get(s1,'Position')-[0 -deltax+.03 0 0]);
% h2=pcolor(time,squeeze(data.beamalt(:,1)/1000),NeNorth);
h2=pcolor(time,zaxis,DataCut2');
hold on;
ax2 = gca;
drawnow; pause(0.1);
yruler = ax2.YRuler;
...
% Plot Te
s4=subplot('Position', get(s3,'Position')-[0 -deltax+0.03 0 0]);
% set(s4, 'Position', );
h4=pcolor(time,zaxis,DataCut4');
hold on;
ax4 = gca;
drawnow; pause(0.2);
yruler = ax4.YRuler;
...
shading flat;
set(ax4,'Layer','top')
y4=ylabel('Altitude (Km)','fontsize',fontsize);
caxis([10 11.5]);
colormap(jet)
ylim([140 700]);

% Plot Te
s5=subplot('Position', get(s4,'Position')-[0 -deltax+0.03 0 0]);
h5=pcolor(time,zaxis,DataCut5');
hold on;
title('Electron density at 100km East/[500 400 300 200 100]km North of RISRN')
ax5 = gca;
drawnow; pause(0.2);
yruler = ax5.YRuler;
yruler.Axle.Visible = 'off';
...
shading flat;
set(ax5,'Layer','top')
y5=ylabel('Altitude (Km)','fontsize',fontsize);
caxis([10 11.5]);
colormap(jet)
ylim([140 700]);
...
minx=max([y1pos(1),y2pos(1),y3pos(1),y4pos(1),y5pos(1)]);
...
set(cb1, 'Position', [hpos(1)-0.05 hpos(2) 2*hpos(3)/3 0.95],'fontsize',12)
set(cb1,'Ticks',linspace(10,11.5,16))
set(cb1,'Fontsize',fontsize)
% 
% hpos2=get(cb2,'Position');
% set(cb2, 'Position', [hpos(1)-0.05 hpos2(2) .0581/5 hpos2(4)*2.15],'fontsize',12)
...
timeticks=linspace(timem(1),timem(end),ticks);
...
xticklabels(ax3,[]);
xticks(ax4,timeticks);
xticklabels(ax4,[]);
xticks(ax5,timeticks);
xticklabels(ax5,[]);
% 
% print(fig,'AMISR1Dcut','-fillpage','-dpdf','-r300','-opengl')
%     fig.PaperUnits='inches';
%     fig.PaperPosition = [0 0 8 8.5];
